          ...
          <div class="card border-0">
            ...
            <div class="card-body border bg-white p-4">     
              ...
              <ul class="entry-meta list-unstyled  mb-2" style="list-style-type:circle">
                  @foreach($tags as $tag)
                  <li >
                    <a class="link-primary text-decoration-none" href="#!">{{$tag}}</a>
                  </li>
                @endforeach
                  </ul>
            </div>
            <div class="card-footer border border-top-0 bg-white p-4">
              <ul class="entry-meta list-unstyled d-flex align-items-center m-0">
                ...
                <li>
                  <form method="POST" action="{{ route('EditBlog', $blog->id) }}">
                  @csrf
                      <button class="btn btn-primary ms-4" type="submit">Edit</button>
                  </form>
                </li>
                <li>
                  <form method="POST" action="{{ route('DeleteBlog', $blog->id) }}">
                  @csrf
                      <button class="btn btn-danger ms-4" type="submit">Delete</button>
                  </form>
                </li>
              </ul>
            </div>
